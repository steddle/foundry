...
| `client` | An invokable class of the imprint's own, handed the request, returning `['name' => ..., 'email' => ?string, 'palette' => ?string, 'lockup' => ?url, 'icons' => ?url, 'ink' => ?colour]` or null. The login page's heading and description, and the mail, sign in to `name`, and the mail signs off as it; the mail comes from `name` on `mail.from.address`; `email` fills the address field. `foundry:layouts.auth` takes the rest, on the login page and wherever else it renders, the link's confirm page and the consent screen among them: `palette` on the page, `lockup` as an image over the card, `icons` as the base URL of `favicon.svg`, `favicon-96x96.png` and `apple-touch-icon.png`, `ink` as the browser chrome, and `name` in `<title>`. Null, or a key that is null, leaves the imprint's own |
