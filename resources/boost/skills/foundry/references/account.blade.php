...
- `foundry.account.callback` (GET `foundry/account/callback`) trades the code for the account at `/oauth/token`, with the secret and the verifier. It finds the row by `steddle_id`, else a row with the address and no `steddle_id`, which it links, else it makes one; sets `email`, `name`, `locale` where the table has the column, and `email_verified_at`; signs it in remembered; puts `['kind' => 'passkey'|'magic_link'|'session', 'link_id' => ?string]` in the session under `credential`, what the account signed in with, by reference; and goes to the intended page, else `imprint.account.home`, else `/`. A reader who turns the consent down (`error=access_denied`) goes back to the page they came from where a guest may open it, else home. Any other error from auth, a state not the session's or auth unreachable shows a page that offers to sign in again, and signs no one in: there is no way in while auth is down, and a signed-in session keeps working.
