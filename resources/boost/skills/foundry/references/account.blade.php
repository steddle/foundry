...
- `foundry.account.events` (POST `foundry/account/events`, no session, no CSRF) takes what auth tells the imprint, signed with the client's secret: `Steddle-Timestamp` and `Steddle-Signature: sha256=` over `timestamp.body`, refused at 403 when it does not match or is more than 300 seconds off. `updated` fills the row's address, name and language; `left` runs `Steddle\Foundry\Auth\DeleteAccount`; `signed-out` deletes the account's rows in `sessions` and rotates its remember token. An account the imprint has no row for answers 200.
- `Steddle\Foundry\Account::identify($email, $name)` asks auth for the account at an address, made there where it has none, and returns the linked row: for a recipient, or a send without signing in.
