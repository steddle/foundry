...
{{--
    The groups of foundry:side-nav, each a nav under its title.

    @prop groups title => list of ['label' => …, 'href' => …, 'current' => bool], as foundry:side-nav takes them.
--}}

<div {{ $attributes->class('flex flex-col gap-8') }}>
    @foreach ($groups as $title => $links)
        <nav aria-label="{{ $title }}" class="flex flex-col gap-3">
            <foundry:text variant="label" tone="muted">{{ $title }}</foundry:text>
