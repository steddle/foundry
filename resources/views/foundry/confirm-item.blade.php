...
{{--
    foundry:confirm-button as a row in a menu: three presses, the words going
    ...
    <flux:dropdown>
        ...
        <flux:menu>
            ...
            <flux:menu.separator />
            <foundry:confirm-item icon="trash" action="$el.dataset.done = 'yes'">Delete</foundry:confirm-item>
        </flux:menu>
    </flux:dropdown>
--}}
<div class="contents" x-data="{ step: 0, timer: null }"
    ...
    <flux:menu.item variant="danger" :$icon x-bind:data-step="step" {{ $attributes->class('relative isolate overflow-hidden text-danger-700! dark:text-danger-300! **:data-flux-menu-item-icon:text-current!') }}>
        ...
        <foundry:confirm-button.labels :label="$slot" align="start" />
    </flux:menu.item>
