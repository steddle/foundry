{{--
    ...
    @example On ink
    @ground ink
    <foundry:service-line />
--}}
<div {{ $attributes->class('flex flex-wrap items-baseline justify-between gap-x-8 gap-y-3') }}>
    <foundry:text variant="small" tone="muted">{{ __(config('imprint.disclaimer')) }}</foundry:text>
    <foundry:text variant="small" tone="muted" class="flex flex-wrap items-baseline gap-x-2 tabular-nums">
