...
{{--
    ...
    @ground bare
    <foundry:sections.split eyebrow="The book" title="Graded before it counts." lead="Every claimant passes four checks, and every check keeps its date and its source.">
        <x-slot:figure>
            <div class="h-64 rounded-lg border border-zinc-200 bg-zinc-25 dark:border-zinc-700 dark:bg-zinc-800"></div>
        </x-slot:figure>
    </foundry:sections.split>

    @example On a scene, with an action
    @ground bare
    <foundry:sections.split :scene="array_key_first(config('imprint.scenes'))" eyebrow="For agents" title="Send one from Claude." lead="Connect it once, and your assistant does the rest.">
        <x-slot:actions>
            <foundry:button href="#">Connect</foundry:button>
        </x-slot:actions>
        <x-slot:figure>
...
--}}
<foundry:section :$scene :$sunken {{ $attributes }}>
