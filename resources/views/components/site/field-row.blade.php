...
{{--
    ...
    x-site.rows does or with `divide-y`.
    ...
    <div class="divide-y divide-zinc-200 border-y border-zinc-200 dark:divide-zinc-700 dark:border-zinc-700">
        <x-site.field-row label="Name" description="How your name appears on every agreement you send." for="name">
            <flux:input id="name" value="Ada Visser" />
        </x-site.field-row>
        <x-site.field-row label="Email" description="Where signing links and signed copies go." for="email">
...
<div {{ $attributes->class('grid gap-3 py-6 md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] md:gap-12') }}>
    <div class="flex flex-col gap-1">
        ...
        @endif
        @if ($description)
            <x-site.text variant="small" tone="muted" class="max-w-[48ch]">{{ $description }}</x-site.text>
        @endif
    </div>
    <div class="min-w-0">
        {{ $slot }}
